      ...
      <div>
        <ul className="hidden mdl:inline-flex items-center gap-6 lg:gap-10">
          {navLinksdata.map(({ _id, title, link }) => (
            <li
              className="text-base font-normal text-gray-400 tracking-wide cursor-pointer hover:text-[#00FA9A] duration-300"
              key={_id}
            >
              <Link
                activeClass="active"
        ...
        <span
          ...
          className="text-xl mdl:hidden bg-black w-10 h-10 inline-flex items-center justify-center rounded-full text-[#00FA9A] cursor-pointer"
        >
          <FiMenu />
        </span>
        {showMenu && (
